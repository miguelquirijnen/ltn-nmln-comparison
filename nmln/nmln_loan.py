    ...
    with np.printoptions(precision=2, suppress=True):
        for i in range(0, 200000):
            ...
            for n,p in o.predicates.items():
                if p.name == "educated":
                    print(
                        "Predicate marginal table for relation educated (e.g. [i] = probability(educated(i)) " )
                    print(np.reshape(M[:, o._predicate_range[n][0]:o._predicate_range[n][1]],
                                     [d.num_constants for d in p.domains]))
                    print()
                elif p.name == "wealthy":
                    print(
                        "Predicate marginal table for relation wealthy (e.g. [i] = probability(wealthy(i)) " )
                    print(np.reshape(M[:, o._predicate_range[n][0]:o._predicate_range[n][1]],
                                     [d.num_constants for d in p.domains]))
                    print()
                elif p.name == "married":
                    print("Predicate marginal table for relation married (e.g. [i,j] = probability(married(i,j)) " )
                    print(np.reshape(M[:, o._predicate_range[n][0]:o._predicate_range[n][1]], [d.num_constants for d in p.domains]))
                    print()
                else: # loan
                    print(
                        "Predicate marginal table for relation loan (e.g. [i] = probability(loan(i)) " )
                    print(np.reshape(M[:, o._predicate_range[n][0]:o._predicate_range[n][1]],
                                     [d.num_constants for d in p.domains]))
                    print()
